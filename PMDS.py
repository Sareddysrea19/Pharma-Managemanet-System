...
def deleteitem():
    e1=entry1.get()
    with open(r"database_proj") as f, open(r"database_proj11", "w") as working:
        for line in f:
            if str(e1) not in line:
                working.write(line)
    os.remove(r"database_proj")
    os.rename(r"database_proj11", r"database_proj")
    f.close()
    working.close()
    entry1.delete(0, END)
    entry2.delete(0, END)
...
def updateitem():
    ...
    e4 = entry4.get()
    e5 = entry5.get()
    with open(r"database_proj") as f1, open(r"database_proj11", "w") as working:
        for line in f1:
            if str(e1) not in line:
                working.write(line)
            else:
